"""Archived generator and predictor for the withdrawn x/y diketone series.

This module preserves the exact pre-withdrawal workflow for provenance. It is
not imported by the current a--f-only reproduction pipeline and is not kept in
sync with the active model API. The corresponding frozen inputs and result
snapshot are stored under ``data/archive/xy_diketones``.
...
from libs import current_model, diketone_metrics  # noqa: E402
...
def select_series_rows(excel_path: Path, series: str) -> pd.DataFrame:
    ...
    rows = workbook.load_experimental_dataset(excel_path).copy()
...
def ensure_quantum_files(
    ...
    for row in unique.itertuples(index=False):
        ...
        # but before quantum_runner writes ``done``.  The eCoMFA descriptor only
        ...
        has_ee_cubes = bool(opt_logs) and all(
            quantum_runner.normal_termination(log)
    ...
    def calculate(record: dict[str, object]) -> str:
        ...
        quantum_runner.calc_ket(
            str(record["molecule_dir"]),
            str(record["SMILES"]),
            quantum_runner.GAUSSIAN_RUN_COMMAND,
        )
...
def main() -> None:
    ...
    from libs.current_model_support.diketone_plots import (  # noqa: PLC0415
