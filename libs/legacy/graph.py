"""Legacy plotting utilities retained for analysis provenance.

The historical three-field analysis accumulated many plotting and kinetic
helpers in this module. The accepted model uses the dedicated helpers in
``libs/current_model_support``; fitting and validation live in
``libs/current_model.py``. Historical diketone summary kinetics delegate to
the tested implementation in ``libs/diketone_metrics.py``.
"""
...
def _simulate_diketone_selectivity(
    ...
    try:
        from libs.diketone_metrics import simulate_full  # noqa: PLC0415
    except ImportError:
        # ``current_model.py`` also supports legacy execution with ``libs``
        # directly on sys.path, where ``graph`` has no package parent.
        from libs.diketone_metrics import (  # type: ignore[no-redef]  # noqa: PLC0415
            simulate_full,
        )
...
def save_diketone_selectivity_summary(
    df: pd.DataFrame,
    save_path: str | Path = "data/legacy/test/selectivity_summary.csv",
) -> pd.DataFrame:
...
if __name__ == '__main__':
    ...
    save_diketone_selectivity_summary(df, "data/legacy/test/selectivity_summary.csv")
